...
Comment la résilience des récifs coralliens, soit la capacité à ne pas s'effondrer suite a un stresse ou une perturbation varie-t-elle selon l'effort de pêche, l'impact des ouragans et la rugosité initiale?
...
Modèle Nondimentionner par rapport aux taux de croissance des poisson perroquets par année avec rugosité fixe:  
...
image(1:nrow(coral), 1:ncol(coral), t(coral), col = plasma(40),
      xlab = "effort de pêche", ylab = "R", main = "Graphique de C finale",axes=F);axis(side = 1, at = 1:ncol(coral), labels = col_names);axis(side = 2, at = 1:nrow(coral), labels = row_names)
```
...
rugfin[,1]<-c(graph.imageR(1,1/9,10,0.1,F,T),
              graph.imageR(2,1/9,10,0.1,F,T),
              graph.imageR(3,1/9,10,0.1,F,T))
rugfin[,2]<-c(graph.imageR(1,1/15,10,0.1,F,T),
              graph.imageR(2,1/15,10,0.1,F,T),
              graph.imageR(3,1/15,10,0.1,F,T))
rugfin[,3]<-c(graph.imageR(1,1/30,10,0.1,F,T),
              graph.imageR(2,1/30,10,0.1,F,T),
              graph.imageR(3,1/30,10,0.1,F,T))
