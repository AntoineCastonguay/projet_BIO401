...
<<<<<<< HEAD
Comment la résilience des récifs coraliens varie-t-elle en fonction de l'effort de pêche et de l'effet des ouragans?
...
=======
Comment la résilience (persistance) des récifs coralliens varie-t-elle selon l'effort de pêche, l'impact des ouragans et la rugosité?

### Hypothèse :

1. La résilience diminue quand l'effort de peche augmente.  
2. La résilience diminue quand les ouragan sont plus fréquence. 
3. La résilience diminue quand les ouragan sont plus puissants.  
4. La résilience diminue quand la rugosité est faible.  
>>>>>>> 16ac0c38c333d22f9eb07e8351ac62ae63201eac
...
#### Tableau 2 : Variables utilisées dans le modèle de référence avec leur définition anisi que leurs limites.
| Variable | Valeurs possibles | Définition |
|:--:|:----:|:--------------|
| C | entre 0 et 1 | Correspond à la proportion de couverture du paysage par le corail. Où 1 est un couvert total et où 0 est l'absence de corail. |
| M | entre 0 et 1 | Correspond à la proportion de couverture du paysage par les macroalgues. Où 1 est un couvert total et où 0 est l'absence des macroalgue. |
| P | entre 0 et 1 | Correspond à la taille de population des poisson perroquets relativement à leur capacité de soutien. Vaut 1 lorsque la capacité de support est atteinte et 0 si les poissons perroquets sont absents |
| R | entre 1 et 3 | Correspond à la rugosité des récifs coralliens, où 1 équivaut à une structure parfaitement lisse et où 3 représente une complexité tridimensionelle prononcée, rarement atteinte, qui offrant une très bonne qualité d'habitat. |
| Y | entre 0 et 1 | Correspond à la proportion de couverture du paysage par du gazon d'algue, remplit l'espace inoccupé par le corail ou les macroalgues. |
</div>

### Méthode :
<<<<<<< HEAD
Modèle pour une rugosité fixe:  
...
La capacité de support de l'environnement pour le poisson perroquets prend en compte l'abondance des algues, qui constituent leur nourriture, ainsi que la rugosité, qui constitue de l'habitat K(M + T, R).  
$\small{K(M+T,R) = }\frac{(c1R + c2)}{K_{max}}\frac{\delta(M+Y)}{1 + v(M+Y)}$

Modèle Nondimentionner par rapport taux de croissance des poisson perroquet par année avec rugosité fixe:  
...
<<<<<<< HEAD
Modèle non-dimensionalisé par rapport taux de croissance des poisson perroquet par année:  
$\frac{dC}{dts} = (\small{rTC - dC - aMC})\frac{1}{s}$   
$\frac{dM}{dts} = (\small{aMC} - \frac{\alpha \frac{P}{\beta}M}{M + Y} - \small{\gamma MY})\frac{1}{s}$   
$\frac{dP}{dts} = \small{P(1 -} \frac{P}{\beta K(M+T,R)} \small{ - fP}$  
...
Ajout de composantes d'ouragans stochastiques:  
=======
...
9. Les effets des ouragans sont immédiat et instantané sur les population de l'écosystème.
...
11. Un ouragan affecte l'ensemble de l'écosysteme de facon égale. 
12. Les ouragan suit un processus de saut de Poisson avec un taux de saut λ tel que dq = 0 avec une probabilité de 1 - λdt et dq = 1 avec une probabilité de λdt.
13. Tous les parametres du modele sont représentatif de la situation réel de l'écosysteme.
...
```{r, eval=F}
# Fonction pour le modele nondimentionner par rapport taux de croissance des poisson perroquet par année avec rugosité fixe : 
...
Après analyse des résultats, on remarque facilement que la pêche est le facteur qui a l'effet le plus clair sur le corail, réduisant grandement la possibilité que le corail se rétablisse. Il semble que le couvert de corail soit légèrement moins résilient à la pêche quand les ouragans sont moins fréquents et de faible intensité, mais cette tendance s'inverse quand les impacts sont modérés ou importants. La rugosité, elle, arrive aussi, semble supporter de plus grands niveaux de pêche sans s'effondrer quand les ouragans sont plus fréquents quand l'impact est faibles, bien que cette tendance semble s'estomper quand l'impact augmente. Toutefois quand les ouragans sont moins fréquents et violents et que l'effort de pêche est assez bas, la rugosité et le couvert de corail se rétablit à des niveaux plus hauts. Ces résultats pointent vers l'importance de considérer l'application de mesures de restriction à la pêche dans les zones ou les ouragans sont moins fréquents et/ou plus violents, car le corail est plus fragile dans ces endroits et car le gain d'une petite réduction de l'effort de pêche peut permettre une amélioration importante de l'état du corail, donc une meilleure efficacité de conservation. 
